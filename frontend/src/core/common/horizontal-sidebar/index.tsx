...
const HorizontalSidebar = () => {
    ...
    const visibleHorizontalSidebarData = isStartupSession
      ? HorizontalSidebarData.map((section: any) => ({
          ...section,
          menu: (section?.menu || []).map((item: any) => {
            let nextItem = item;
            if (item?.menuValue === "Dashboard") {
              nextItem = {
                ...nextItem,
                subMenus: (nextItem?.subMenus || []).filter((sub: any) =>
                  ["Startup Dashboard", "Indicateurs S2T"].includes(sub?.menuValue)
                ),
              };
            }
            if (!isExpertSession) {
              nextItem = {
                ...nextItem,
                subMenus: (nextItem?.subMenus || []).filter(
                  (sub: any) => sub?.base !== "notes" && sub?.menuValue !== "Notes"
                ),
              };
            }
            if (!isStartupSession) {
              nextItem = {
                ...nextItem,
                subMenus: (nextItem?.subMenus || []).filter(
                  (sub: any) => sub?.base !== "experts-list" && sub?.menuValue !== "Experts"
                ),
              };
            }
            return nextItem;
    ...
    <div className="sidebar sidebar-horizontal" id="horizontal-menu">
        <div className="sidebar-menu">
            <div className="main-menu">
                ...
                {visibleHorizontalSidebarData?.map((mainMenu: any, index: number) => (
                <React.Fragment key={`main-${index}`}>
                    {mainMenu?.menu?.map((data: any, i: number) => (
                    <li className="submenu" key={`menu-${i}`}>
                        ...
                        {data?.subMenus?.map((subMenu:any, j: number) => (
                            <li
                            key={`submenu-${j}`}
